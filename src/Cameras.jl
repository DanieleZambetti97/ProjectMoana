...
**Ray** represent a light ray starting form the `orgin`(Point) in the `dir`(Vec) direction.
`tmin` and `tmax` indicate the maximun and minimum distance from the orign that will be considered.
`depth` reperesents the number of reflection made by the ray.
"""
struct Ray
    origin::Point 
    dir::Vec
...
Represent the screen from which is observed the scene.
"""
