...
@testset "Renderer: Furnace test for Path Tracer renderer" begin
    ...
    for i ∈ 1:1e3
        ...
        path_tracer = PathTracer_Renderer(world, pcg=pcg, num_of_rays=1, max_depth=1000, russian_roulette_limit=1001)

        ray = Ray(Point(0, 0, 0), Vec(1, 0, 0))
        color = PathTracer(ray, path_tracer)

        expected = emitted_radiance / (1.0 - reflectance)
        @test isapprox(expected, color.r, atol=5e-3)
        @test isapprox(expected, color.g, atol=5e-3)
        @test isapprox(expected, color.b, atol=5e-3)
    end
